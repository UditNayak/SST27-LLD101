...
public class EmployeeRepository {
    List<Employee> getEmployeesList(){
        ...
        // Wrap all legacy objects with their respective adapters
        List<Employee> allEmployees = new ArrayList<>();
        for (EmployeeCSV csv : csvRows) {
            allEmployees.add(SimpleEmployeeFactory.createEmployee("CSV", csv));
        }
        for (EmployeeDB db : dbRows) {
            allEmployees.add(SimpleEmployeeFactory.createEmployee("DB", db));
        }
        for (EmployeeLDAP ldap : ldapRows) {
            allEmployees.add(SimpleEmployeeFactory.createEmployee("LDAP", ldap));
        }
